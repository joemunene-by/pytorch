...
class _PrecompileApi:
    ...
    def load(self, python_code: str, cache: bytes) -> PrecompiledModule:
        ...
        # The cache carries only the compiled inductor artifact: use it for the fast
        # reconstruction, else fall back to exec'ing the self-contained python_code.
        # The cache is unpickled with weights_only=False (it holds an executable
        # compiled artifact, which weights_only/add_safe_globals cannot make safe), so
        # warn that loading it runs embedded code -- only load trusted caches.
        from torch._logging import warning_once

        warning_once(
            log,
            "torch.precompile.load deserializes the cache with weights_only=False, "
            "which executes code embedded in it. Only load caches you produced or "
        ...
        # The reloaded artifact runs from python_code / the inductor cache; the live
        # captured GraphModule is not reconstructed (inspect it via python_code).
        obj._gm = None
