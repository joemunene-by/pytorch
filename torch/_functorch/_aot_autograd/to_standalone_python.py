...
def compile_to_python(
    ...
    *,
    ...
    THREADING: this must run single-threaded per process. The wrapper-source capture
    is thread-local, but the underlying ``standalone_compile`` enters
    ...
    artifacts. Do not call this (or ``torch.precompile``) from multiple threads at once.
    """
