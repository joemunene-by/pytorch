...
def _extract_inductor_output_module(artifact: CompiledArtifact) -> str:
    ...
    one exposing ``call``. ``compile_to_python`` disables ``benchmark_harness`` so the
    module carries no get_args/benchmark/``__main__`` debug block; the trailing
    ``__main__`` strip below is a defensive fallback in case one slips through.
    """
    chunks: list[str] = []
